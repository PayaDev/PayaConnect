...
| account_vault_api_id            | string     |     | 36    |               | ✔            | ✔           |      | This can be supplied in place of account_vault_id if you would like to use an account vault for the transaction and are using your own custom api_id's to track accountvaults in the system. **Required** with [AccountForm](https://github.com/PayaDev/PayaConnect/tree/master/AccountForm)                                                                                                                                                                                                              |
...
| ach_sec_code                    | string     | 3   | 3     |               | ✔            |             | N    |  Required for ACH transactions. See the Merchant ACH section for more info                                                                                                                                                                                                                                                                                              |
...
| billing_state                   | string     |     | 26     | ✔            | ✔            |             | N    | The valid two character state (e.g. MI, OH, NY) State portion of the address associated with the Credit Card (CC) or Bank Account (ACH). **Only required for Canadian Merchant Processing**                                                                                                                                                                                                                                                                                                              |
...
| previous_transaction_id         | string     |     | 36    |               | ✔            |             | N    | previous_transaction_id is uses the original transaction_id as token to run a transaction. Account details OR previous_transaction_id should be passed to run a transaction.                                                                                                                                                                                                                                                               |
| previous_transaction_api_id         | string     |     | 36    |               | ✔            |             | N    | previous_transaction_api_id uses the transaction_api_id as a token to run transactions. Account details OR previous_transaction_api_id should be passed to run transactions. **Note: If you are using this to run a new transaction due to a timeout or server error, please query the transaction_api_id before attempting a new authonly or sale.**                                                                                                                                                                                                                                                               |
...
| service_address                  | string     |     | 32    |               | ✔            |             | N    | The Street portion of the service address associated with the Credit Card (CC) if different than the billing street. Only applies to MC transactions.                                                                                                                                                                                                  |
| service_city                    | string     |     | 36    |               | ✔            |             | N    | The City portion of the address associated with the Credit Card (CC) if different than the billing city. Only applies to MC transactions.                                                                                                                                                                                                                                        |
| service_country                    | string     |     | 3    |               | ✔            |             | N    | The 3 digit IBAN code for the service country. Only applies to MC transactions (USA = 840).                                                                                                                                                                                                                                                                                                                |
| service_state                   | string     |     | 26     | ✔            | ✔            |             | N    | The valid two character state (e.g. MI, OH, NY) State portion of the address associated with the Credit Card (CC) if different than the billing state. Only applies to MC transactions.                                                                                                                                                                                                                      |
| service_zip                     | string     | 4   | 12    |               | ✔            |             | N    | The Zip or "Postal Code" portion of the address associated with the Credit Card (CC) if different than the billing zip. Only applies to MC transactions. Alphanumeric with spaces and dashes to accomodate domestic and international postal codes.                                                                                              |
...
| subtotal_amount                 | number     |     | 10    |               | ✔            | ✔          |      | This field is allowed and required for transactions (action = Sale, AuthOnly, and AuthComplete) that have a product where surcharge and or Auto Calculate Tax is configured. This amount should not include the value of the "tax" field (sales tax) if present. |
| surcharge_amount                | number     |     | 10    |               | ✔            | ✔           |      | This field is allowed and required for transactions (action = Sale, AuthOnly, and AuthComplete) that have a product where surcharge is configured. |
| tags                            | string     |     |       |               | ✔            | ✔           | O    |                                                                                                                                                                                                                                                                                                                                                                                                            |
| tax                             | number     |     |       |               | ✔            | ✔           |      | Amount of Sales tax - If supplied, this amount should be included in the total transaction_amount field and is required for transactions where Auto Calculate Tax is configured. |
...
| transaction_api_id              | string     |     | 64    |               | ✔            |             | N    | See api_id page for more details. **Required** with [PayForm](https://github.com/PayaDev/PayaConnect/tree/master/PayForm)                                                                                                                                                                                                                                                                                                                                                                           |
...
| subtotal_amount                 |      |        |      |                 |          |              |               |       |           |       |        |      | This field is allowed and required for transactions that have a product where surcharge is configured. This amount should not include the value of the "tax" field (sales tax) if present.                                                                                                                                                                                                                                                                                                     |
...
```json
{
        "transaction": {
            ...
            "transaction_amount": "66.80",
            "subtotal_amount": "60.00",
            "tax": "5.00",
            "surcharge_amount": "1.8"
    }
}
```
